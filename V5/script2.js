...
function projectManager(ctx) {
	...
	console.log("project_all:", ctx.project_all);
	// console.log("beside_target:", ctx.beside_target);
...
function repositionGrid(gridEl, newSnap, side, instant = false) {
	...
	const preTransform = `
      translateX(${baseTranslateX})
      translateZ(calc(var(--scroll-depth) - var(--halfSizeWall) + var(--sizeWall) * ${factor + 1}))
      ${baseRotateY}
    `;
...
function handleClick(e) {
	...
	if (close_btn) {
		...
		closeGrid({ grid:document.querySelector(".grid.active"), inner:document.querySelector(".grid.active .inner"), scene });
		return;
	}
