...
document.querySelectorAll(".volet").forEach((el) => {
  el.addEventListener("mouseenter", () =>
    canvasManager.hover(el.dataset.index, true)
  );
  el.addEventListener("mouseleave", () =>
    canvasManager.hover(el.dataset.index, false)
  );
});
...
function focusOnGrid(ctx) {
  ...
  setTimeout(() => {
    ...
    projectManager({
      ...
      target: ctx.gridElement.dataset.snap,
...
function projectManager(ctx) {
  ...
  ctx.project = document.querySelector(
    `.projects [data-target="${ctx.target}"]`
  );
  ctx.project.classList.add("p-focus");
  ...
  console.log("beside_target after:", ctx.beside_target);

  ctx.scrollZone.scroll({
    top: ctx.project.offsetTop,
    behavior: "instant",
  });
  ctx.video = ctx.project.querySelector("video");
      ...
      entries.forEach((entry) => {
        ...
        if (video) {
          if (entry.isIntersecting) {
            video.play().catch((err) => console.log("Vid play error:", err));
          } else {
            video.pause();
          }
        }
  ...
  const newobserver = new IntersectionObserver(
    ([entry]) => {
      ...
      if (entry.intersectionRatio < 1) {
        ...
        entry.target.closest(".project").classList.remove("p-focus");
        ctx.scrollZone.scroll({
          top: ctx.project.offsetTop,
          behavior: "instant",
        });
    ...
    },
    {
      ...
      threshold: [1], // déclenche quand 50% ou plus est visible
    }
  );

  newobserver.observe(ctx.video);
...
function repositionGrid(gridEl, newSnap, side, instant = false) {
  ...
  const finalTransform = `
      ...
      translateZ(calc(var(--scroll-depth) - var(--halfSizeWall) + var(--sizeWall) * ${factor}))
      ${baseRotateY}
    `;
  if (instant) {
  ...
  const preTransform = `
      translateX(${baseTranslateX})
      translateZ(calc(var(--scroll-depth) - var(--halfSizeWall) + var(--sizeWall) * ${
        factor + 1
      }))
      ${baseRotateY}
    `;
...
function handleClick(e) {
  ...
  const close_btn = e.target.classList == "button";

  if (close_btn) {
    closeGrid({
      grid: document.querySelector(".grid.active"),
      inner: document.querySelector(".grid.active .inner"),
      scene,
    });
    return;
  }
...
function closeGrid({ grid, inner, scene }) {
  ...
  projects.classList.remove("open_projects");
